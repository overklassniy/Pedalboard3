...
void BranchesLAF::drawButtonBackground(Graphics& g, Button& button, const Colour& backgroundColour,
    ...
    g.drawRoundedRectangle(1.0f, 1.0f, static_cast<float>(button.getWidth() - 2),
                           static_cast<float>(button.getHeight() - 2), 4.0f, 1.0f);
    g.setGradientFill(grad);
    g.fillRoundedRectangle(1.0f, 1.0f, static_cast<float>(button.getWidth() - 2),
                           static_cast<float>(button.getHeight() - 2), 4.0f);
    ...
    shadow.lineTo(static_cast<float>(button.getWidth() - 4), static_cast<float>(button.getHeight() - 2));
    shadow.quadraticTo(static_cast<float>(button.getWidth() - 2), static_cast<float>(button.getHeight() - 2),
                       static_cast<float>(button.getWidth() - 2), static_cast<float>(button.getHeight() - 4));
...
void BranchesLAF::drawButtonText(Graphics& g, TextButton& button, bool shouldDrawButtonAsHighlighted,
...
void BranchesLAF::drawScrollbarButton(Graphics& g, ScrollBar& scrollbar, int width, int height, int buttonDirection,
    ...
    if (!isScrollbarVertical) {
        ...
        if (buttonDirection == 3) {
            g.setGradientFill(grad);
            g.fillRect(static_cast<float>(3), static_cast<float>(0), static_cast<float>(width),
            ...
            g.setGradientFill(grad);
            g.fillRect(static_cast<float>(0), static_cast<float>(0), static_cast<float>(width) - 3.0f,
                       static_cast<float>(height));
        ...
        g.drawRoundedRectangle(1.0f, 1.0f, static_cast<float>(width - 2), static_cast<float>(height - 2), 2.0f, 1.0f);
        g.setColour(colours["Button Colour"]);
        g.fillRoundedRectangle(1.0f, 1.0f, static_cast<float>(width - 2), static_cast<float>(height - 2), 2.0f);

        g.setColour(Colour(0x08000000));
        g.fillRoundedRectangle(1.0f, static_cast<float>(height / 2), static_cast<float>(width - 2),
        ...
        if (isMouseOverButton) {
            ...
            shadow.lineTo(static_cast<float>(width - 4), static_cast<float>(height - 2));
            shadow.quadraticTo(static_cast<float>(width - 2), static_cast<float>(height - 2),
                               static_cast<float>(width - 2), static_cast<float>(height - 4));
        ...
        ColourGradient grad(colours["Window Background"].darker(0.25f), 0.0f, 0.0f, colours["Window Background"],
                            static_cast<float>(width), 0.0f, false);

        if (buttonDirection == 2) {
            g.setGradientFill(grad);
            g.fillRect(static_cast<float>(0), static_cast<float>(0), static_cast<float>(width),
            ...
            g.setGradientFill(grad);
            g.fillRect(static_cast<float>(0), static_cast<float>(3), static_cast<float>(width),
                       static_cast<float>(height));
        ...
        g.setColour(Colour(0x08000000));
        g.fillRoundedRectangle(static_cast<float>(width / 2), 1.0f, static_cast<float>(width - 2),
                               static_cast<float>(height - 2), 2.0f);
...
void BranchesLAF::drawScrollbar(Graphics& g, ScrollBar& scrollbar, int x, int y, int width, int height,
    ...
    if (!isScrollbarVertical) {
        ColourGradient grad(colours["Window Background"].darker(0.25f), 0.0f, static_cast<float>(y),
                            colours["Window Background"], 0.0f, static_cast<float>(height), false);
        ...
        g.setColour(Colour(0x80000000));
        g.drawRoundedRectangle(static_cast<float>(thumbStartPosition + 1), static_cast<float>(y + 1),
                               static_cast<float>(thumbSize - 2), static_cast<float>(height - 2), 2.0f, 1.0f);
        g.setColour(colours["Button Colour"]);
        g.fillRoundedRectangle(static_cast<float>(thumbStartPosition + 1), static_cast<float>(y + 1),
                               static_cast<float>(thumbSize - 2), static_cast<float>(height - 2), 2.0f);

        g.setColour(Colour(0x08000000));
        g.fillRoundedRectangle(static_cast<float>(thumbStartPosition + 1), static_cast<float>(height / 2),
                               static_cast<float>(thumbSize - 2), static_cast<float>(height - 2), 2.0f);
    } else {
        ColourGradient grad(colours["Window Background"].darker(0.25f), static_cast<float>(x), 0.0f,
                            colours["Window Background"], static_cast<float>(width), 0.0f, false);
        ...
        g.setColour(Colour(0x80000000));
        g.drawRoundedRectangle(static_cast<float>(x + 1), static_cast<float>(thumbStartPosition + 1),
                               static_cast<float>(width - 2), static_cast<float>(thumbSize - 2), 2.0f, 1.0f);
        g.setColour(colours["Button Colour"]);
        g.fillRoundedRectangle(static_cast<float>(x + 1), static_cast<float>(thumbStartPosition + 1),
                               static_cast<float>(width - 2), static_cast<float>(thumbSize - 2), 2.0f);

        g.setColour(Colour(0x08000000));
        g.fillRoundedRectangle(static_cast<float>(width / 2), static_cast<float>(thumbStartPosition + 1),
                               static_cast<float>(width - 2), static_cast<float>(thumbSize - 2), 2.0f);
    }
}
...
int BranchesLAF::getMenuBarItemWidth(MenuBarComponent& menuBar, int itemIndex, const String& itemText) {
    return juce::GlyphArrangement::getStringWidthInt(getMenuBarFont(menuBar, itemIndex, itemText), itemText) +
           menuBar.getHeight() - 8;
}
...
void BranchesLAF::drawPopupMenuBackground(Graphics& g, int width, int height) {
    ...
    // Draw shadow.
    shadow.startNewSubPath(3.0f, static_cast<float>(height - 2));
    shadow.lineTo(static_cast<float>(width - 2), static_cast<float>(height - 2));
    shadow.lineTo(static_cast<float>(width - 2), 3.0f);
    g.setColour(Colour(0x20000000));
    g.strokePath(shadow, PathStrokeType(1.0f, PathStrokeType::curved, PathStrokeType::rounded));
...
void BranchesLAF::drawComboBox(Graphics& g, int width, int height, bool isButtonDown, int buttonX, int buttonY,
    ...
    g.setColour(Colour(0x80000000));
    g.drawRoundedRectangle(static_cast<float>(buttonX) + 1.0f, static_cast<float>(buttonY) + 1.0f,
                           static_cast<float>(buttonW - 2), static_cast<float>(buttonH - 2), 2.0f, 1.0f);
    g.setColour(colours["Button Colour"]);
    g.fillRoundedRectangle(static_cast<float>(buttonX) + 1.0f, static_cast<float>(buttonY) + 1.0f,
                           static_cast<float>(buttonW - 2), static_cast<float>(buttonH - 2), 2.0f);
    ...
    if (box.isEnabled()) {
        ...
        tri.startNewSubPath(
            static_cast<float>(buttonX) + static_cast<float>(buttonW / 2) - static_cast<float>(buttonW / 4) + inc,
        ...
        tri.lineTo(static_cast<float>(buttonX) + static_cast<float>(buttonW / 2) + inc,
                   static_cast<float>(buttonY) + static_cast<float>(buttonH / 2) + static_cast<float>(buttonH / 8) +
                       inc);
        tri.lineTo(
            static_cast<float>(buttonX) + static_cast<float>(buttonW / 2) + static_cast<float>(buttonW / 4) + inc,
            static_cast<float>(buttonY) + static_cast<float>(buttonH / 2) - static_cast<float>(buttonH / 8) + inc);
...
void BranchesLAF::drawProgressBar(Graphics& g, ProgressBar& progressBar, int width, int height, double progress,
    ...
    // Draw the foreground.
    g.setColour(Colour(0x80000000));
    g.drawRoundedRectangle(1.0f, 1.0f, static_cast<float>(width - 2) * static_cast<float>(progress),
                           static_cast<float>(height - 2), 2.0f, 1.0f);
    g.setColour(colours["CPU Meter Colour"]);
    g.fillRoundedRectangle(1.0f, 1.0f, static_cast<float>(width - 2) * static_cast<float>(progress),
                           static_cast<float>(height - 2), 2.0f);

    g.setColour(Colour(0x08000000));
    g.fillRoundedRectangle(1.0f, static_cast<float>(height / 2),
                           static_cast<float>(width - 2) * static_cast<float>(progress), static_cast<float>(height - 2),
                           2.0f);
...
void BranchesLAF::drawKeymapChangeButton(Graphics& g, int width, int height, Button& button,
    ...
    if (keyDescription.isNotEmpty()) {
        ...
        g.fillPath(p, p.getTransformToScaleToFit(2.0f, 2.0f, static_cast<float>(width) - 4.0f,
                                                 static_cast<float>(height) - 4.0f, true));
    }
}
...
void BranchesLAF::drawLabel(Graphics& g, Label& label) {
    ...
    if (!label.isBeingEdited()) {
        ...
        g.drawFittedText(label.getText(), labelBorder.getLeft(), labelBorder.getTop(),
                         ...
                         label.getHeight() - labelBorder.getTopAndBottom(), label.getJustificationType(),
