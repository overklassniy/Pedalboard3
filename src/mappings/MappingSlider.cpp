...
public:
    PopupDisplayComponent(MappingSlider& owner_)
        : owner(owner_)
        , font(juce::FontOptions().withHeight(15.0f).withStyle("Bold"))
    {
        setAlwaysOnTop(true);
    }
...
namespace MappingSliderHelpers
{
    double smallestAngleBetween(double a1, double a2) noexcept
    {
        return jmin(std::abs(a1 - a2),
                    std::abs(a1 + juce::MathConstants<double>::pi * 2.0 - a2),
            ...
            else
            {
                ...
                if (speed != 0)
                {
                    speed = 0.2 * velocityModeSensitivity
                        * (1.0 + std::sin(juce::MathConstants<double>::pi * (1.5 + jmin(0.5, velocityModeOffset
                                            ...
                                            MappingSlider& slider)
{
    ...
    if (style == MappingSlider::LinearBar)
    {
        ...
        drawShinyButtonShape(g,
                             static_cast<float>(x), static_cast<float>(y), sliderPos - static_cast<float>(x), static_cast<float>(height), 0.0f,
                             baseColour,
