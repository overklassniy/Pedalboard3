...
class MappingSlider::PopupDisplayComponent : public BubbleComponent, public Timer {
  public:
    PopupDisplayComponent(MappingSlider& owner_)
        : owner(owner_), font(juce::FontOptions().withHeight(15.0f).withStyle("Bold")) {
        setAlwaysOnTop(true);
    }
    ...
    void getContentSize(int& w, int& h) {
        w = juce::GlyphArrangement::getStringWidthInt(font, text) + 18;
        h = static_cast<int>(font.getHeight() * 1.6f);
    }
...
MappingSlider::MappingSlider(const String& name)
    : Component(name), lastCurrentValue(0), lastValueMin(0), lastValueMax(0), minimum(0), maximum(10), interval(0),
      doubleClickReturnValue(0), skewFactor(1.0), velocityModeSensitivity(1.0), velocityModeOffset(0.0),
      velocityModeThreshold(1), rotaryStart(juce::MathConstants<float>::pi * 1.2f),
      rotaryEnd(juce::MathConstants<float>::pi * 2.8f), numDecimalPlaces(7), sliderRegionStart(0), sliderRegionSize(1),
      sliderBeingDragged(-1), pixelsForFullDragExtent(250), style(TwoValueHorizontal), textBoxPos(TextBoxLeft),
      textBoxWidth(80), textBoxHeight(20), incDecButtonMode(incDecButtonsNotDraggable), editableText(true),
      doubleClickToValue(false), isVelocityBased(false), userKeyOverridesVelocity(true), rotaryStop(true),
      incDecButtonsSideBySide(false), sendChangeOnlyOnRelease(false), popupDisplayEnabled(false), menuEnabled(false),
      menuShown(false), mouseWasHidden(false), incDecDragged(false), scrollWheelEnabled(true), snapsToMousePos(true),
...
void MappingSlider::handleAsyncUpdate() {
    ...
    listeners.callChecked(checker, &MappingSliderListener::sliderValueChanged, this);
}
...
void MappingSlider::setRange(const double newMin, const double newMax, const double newInt) {
    if (minimum != newMin || maximum != newMax || interval != newInt) {
        ...
        if (newInt != 0) {
            int v = abs(static_cast<int>(newInt * 10000000));
...
void MappingSlider::setValue(double newValue, const bool sendUpdateMessage, const bool sendMessageSynchronously) {
    ...
    if (style == ThreeValueHorizontal || style == ThreeValueVertical) {
        ...
        newValue = jlimit(static_cast<double>(valueMin.getValue()), static_cast<double>(valueMax.getValue()), newValue);
    }
...
void MappingSlider::setMinValue(double newValue, const bool sendUpdateMessage, const bool sendMessageSynchronously,
    ...
    juce::ignoreUnused(allowNudgingOfOtherValues);
...
String MappingSlider::getTextFromValue(double v) {
...
float MappingSlider::getLinearMappingSliderPos(const double value) {
    ...
    return static_cast<float>(sliderRegionStart + sliderPosProportional * sliderRegionSize);
}
...
void MappingSlider::paint(Graphics& g) {
    if (style != IncDecButtons) {
        if (style == Rotary || style == RotaryHorizontalDrag || style == RotaryVerticalDrag) {
            const float sliderPos = static_cast<float>(valueToProportionOfLength(lastCurrentValue));
    ...
    if (isEnabled()) {
        if (e.mods.isPopupMenu() && menuEnabled) {
            ...
            if (style == TwoValueHorizontal || style == TwoValueVertical || style == ThreeValueHorizontal ||
                style == ThreeValueVertical) {
                const float mousePos = static_cast<float>(isVertical() ? e.y : e.x);
            ...
            if (popupDisplayEnabled) {
                auto* const popup = new PopupDisplayComponent(*this);
...
void MappingSlider::restoreMouseIfHidden() {
    if (mouseWasHidden) {
        ...
        const double pos =
            (sliderBeingDragged == 2)
                ? getMaxValue()
                : ((sliderBeingDragged == 1) ? getMinValue() : static_cast<double>(currentValue.getValue()));
        ...
        if (style == RotaryHorizontalDrag || style == RotaryVerticalDrag) {
            ...
            const int pixelPos = static_cast<int>(getLinearMappingSliderPos(pos));
...
void MappingSlider::modifierKeysChanged(const ModifierKeys& modifiers) {
    if (isEnabled() && style != IncDecButtons && style != Rotary &&
...
double smallestAngleBetween(double a1, double a2) noexcept {
    return jmin(std::abs(a1 - a2), std::abs(a1 + juce::MathConstants<double>::pi * 2.0 - a2),
                std::abs(a2 + juce::MathConstants<double>::pi * 2.0 - a1));
}
} // namespace MappingSliderHelpers
    ...
    if (isEnabled() && (!menuShown) && (maximum > minimum)) {
        if (style == Rotary) {
            ...
            if (dx * dx + dy * dy > 25) {
                ...
                while (angle < 0.0)
                    angle += juce::MathConstants<double>::pi * 2.0;
                ...
                if (rotaryStop && !e.mouseWasClicked()) {
                    ...
                    if (angle >= lastAngle)
                        angle = jmin(angle, static_cast<double>(jmax(rotaryStart, rotaryEnd)));
                    else
                        angle = jmax(angle, static_cast<double>(jmin(rotaryStart, rotaryEnd)));
                } else {
            ...
            if ((isVelocityBased == (userKeyOverridesVelocity
                ...
                double scaledMousePos = (mousePos - sliderRegionStart) / static_cast<double>(sliderRegionSize);
                ...
                double speed = jlimit(0.0, maxSpeed, static_cast<double>(abs(mouseDiff)));

                if (speed != 0) {
                    speed =
                        0.2 * velocityModeSensitivity *
                        (1.0 +
                         std::sin(juce::MathConstants<double>::pi *
                                  (1.5 + jmin(0.5, velocityModeOffset +
                                                       jmax(0.0, static_cast<double>(speed - velocityModeThreshold)) /
                                                           maxSpeed))));
...
void MappingSliderListener::sliderDragStarted(MappingSlider*) {}
...
class SliderLabelComp : public Label {
  public:
    ...
    void mouseWheelMove(const MouseEvent&, const MouseWheelDetails& wheel) override { juce::ignoreUnused(wheel); }
};

Label* MappingSlider::createMappingSliderTextBox(MappingSlider& slider) {
    auto* const l = new SliderLabelComp();
...
void MappingSlider::drawLinearMappingSlider(Graphics& g, int x, int y, int width, int height, float sliderPos,
    ...
    if (style == MappingSlider::LinearBar) {
        ...
        drawShinyButtonShape(g, static_cast<float>(x), static_cast<float>(y), sliderPos - static_cast<float>(x),
                             static_cast<float>(height), 0.0f, baseColour, slider.isEnabled() ? 0.9f : 0.3f, true, true,
                             true, true);
    } else {
...
void MappingSlider::drawLinearMappingSliderBackground(Graphics& g, int x, int y, int width, int height,
                                                      ...
                                                      MappingSlider& slider) {
    const float sliderRadius = static_cast<float>(getMappingSliderThumbRadius(slider) - 2);
...
void drawGlassPointer(Graphics& g, const float x, const float y, const float diameter, const Colour& colour,
    ...
    p.applyTransform(AffineTransform::rotation(direction * (juce::MathConstants<float>::pi * 0.5f), x + diameter * 0.5f,
                                               y + diameter * 0.5f));
