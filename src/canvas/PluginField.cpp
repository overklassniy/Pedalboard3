...
PluginField::PluginField(FilterGraph* filterGraph, juce::KnownPluginList* list,
                         juce::ApplicationCommandManager* appManager)
    : signalPath(filterGraph), pluginList(list), midiManager(appManager), oscManager(appManager),
      draggingConnection(nullptr), tempo(120.0), displayDoubleClickMessage(true) {
    int i;
...
bool PluginField::isInterestedInFileDrag(const juce::StringArray& files) {
    ...
    for (i = 0; i < files.size(); ++i) {
        ...
        else if (files[i].endsWithIgnoreCase(".wav") || files[i].endsWithIgnoreCase(".aif") ||
                 files[i].endsWithIgnoreCase(".aiff") || files[i].endsWithIgnoreCase(".ogg") ||
                 files[i].endsWithIgnoreCase(".flac") || files[i].endsWithIgnoreCase(".wma")) {
            retval = true;
            break;
        }
    }
...
void PluginField::filesDropped(const juce::StringArray& files, int x, int y) {
    ...
    if (pluginsInArray) {
        ...
        for (i = 0; i < foundPlugins.size(); ++i) {
            ...
            signalPath->addFilter(foundPlugins[i], static_cast<double>(x), static_cast<double>(y));
    ...
    if (soundsInArray) {
        for (i = 0; i < files.size(); ++i) {
            ...
            signalPath->addFilter(
                std::unique_ptr<juce::AudioProcessor>(std::make_unique<FilePlayerProcessor>(juce::File(files[i]))),
...
void PluginField::deleteFilter(AudioProcessorGraph::Node::Ptr node) {
    ...
    for (i = getNumChildComponents() - 1; i >= 0; --i) {
        ...
        if (connection) {
            ...
            // Guard against a null destination pin observed in a past crash.
            if (src)
                srcId = src->getUid();
            if (dest)
                destId = dest->getUid();
    ...
    for (it = mappings.lower_bound(uid.uid); it != mappings.upper_bound(uid.uid); ++it) {
        delete it->second;
    }
...
void PluginField::addConnection(PluginPinComponent* source, bool connectAll) {
    if (source) {
        auto* connection = new PluginConnection(source, nullptr, connectAll);
        ...
        if (p) {
            if (!p->getDirection()) {
                ...
                if ((s->getParameterPin() && p->getParameterPin()) ||
                    ...
                    signalPath->addConnection(s->getUid(), s->getChannel(), p->getUid(), p->getChannel());
    ...
    for (i = (getNumChildComponents() - 1); i >= 0; --i) {
        ...
        if (c) {
            if (c->getSelected()) {
                ...
                signalPath->removeConnection(s->getUid(), s->getChannel(), d->getUid(), d->getChannel());
...
void PluginField::enableMidiForNode(AudioProcessorGraph::Node::Ptr node, bool val) {
    ...
    bool connection = signalPath->connectionExists(midiInput->nodeID, AudioProcessorGraph::midiChannelIndex,
...
bool PluginField::getMidiEnabledForNode(AudioProcessorGraph::Node::Ptr node) const {
    ...
    if (!midiInput)
        ...
        return signalPath->connectionExists(midiInput->nodeID, AudioProcessorGraph::midiChannelIndex, node->nodeID,
                                            AudioProcessorGraph::midiChannelIndex);
}
...
void PluginField::loadFromXml(juce::XmlElement* patch) {
    ...
    // Add the audio/midi connections.
    {
        ...
        // Collect all nodes and their ids into an array.
        for (i = 0; i < signalPath->getNumFilters(); ++i) {
            NodeAndId n;
    ...
    // Add in any parameter mapping connections.
    {
        ...
        if (oscInputEnabled && oscInput) {
            // Do the same for the OSC mappings.
...
juce::Component* PluginField::getPinAt(const int x, const int y) {
    ...
    if (isVisible() && static_cast<unsigned int>(x) < static_cast<unsigned int>(getWidth()) &&
        static_cast<unsigned int>(y) < static_cast<unsigned int>(getHeight()) && hitTest(x, y)) {
        for (int i = getNumChildComponents(); --i >= 0;) {
            juce::Component* const child = getChildComponent(i);

            if (!dynamic_cast<PluginConnection*>(child)) {
                juce::Rectangle<int> updatedRect(child->getX() - 16, child->getY() - 16, child->getWidth() + 32,
                                                 child->getHeight() + 32);
                ...
                if (updatedRect.contains(pos)) {
                    ...
                    juce::Component* const c =
                        child->getComponentAt(pos.getX() - child->getX(), pos.getY() - child->getY());
...
void PluginField::connectAll(PluginConnection* connection) {
    ...
    if (source && dest) {
        ...
        for (; (left < numSources) && (right < numDests); ++left, ++right) {
            ...
            signalPath->addConnection(sourcePin->getUid(), sourcePin->getChannel(), destPin->getUid(),
                                      destPin->getChannel());
            addAndMakeVisible(new PluginConnection(sourcePin, destPin));
        }
    }
